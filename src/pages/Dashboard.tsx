...
import { Sprout, ListChecks, Wifi, TrendingUp, Bug, Bot, Settings, Calendar, Calculator, Droplet } from 'lucide-react';
import { motion } from 'framer-motion';
import { pageVariants, cardVariants, staggerContainerVariants, listItemVariants } from '../utils/animations';
...
const Dashboard: React.FC = () => {
  return (
    <motion.div 
      className="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-blue-50 p-0"
    ...
    >
      ...
      >
        <div className="flex flex-col items-center justify-center mb-6">
          <motion.div 
            className="p-4 rounded-full bg-gradient-to-br from-green-500 to-emerald-600 shadow-lg mb-2"
          ...
          <motion.h1 
            className="text-4xl lg:text-5xl font-extrabold high-contrast-text mb-2"
      ...
      <motion.div 
        className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12 px-4"
        variants={staggerContainerVariants}
        initial="initial"
        animate="animate"
      >
        <motion.div variants={cardVariants} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <Link to="/weather" className="enhanced-card p-6 flex flex-col items-center hover:bg-blue-50 smooth-transition group block h-full">
        ...
        <motion.div variants={cardVariants} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <Link to="/market-prices" className="enhanced-card p-6 flex flex-col items-center hover:bg-green-50 smooth-transition group block h-full">
        ...
        <motion.div variants={cardVariants} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <Link to="/crop-management" className="enhanced-card p-6 flex flex-col items-center hover:bg-emerald-50 smooth-transition group block h-full">
        ...
        <motion.div variants={cardVariants} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <Link to="/disease-detection" className="enhanced-card p-6 flex flex-col items-center hover:bg-red-50 smooth-transition group block h-full">
        ...
        <motion.div variants={cardVariants} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <Link to="/settings" className="enhanced-card p-6 flex flex-col items-center hover:bg-gray-50 smooth-transition group block h-full">
        ...
        <motion.div variants={cardVariants} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="sm:col-span-2 lg:col-span-3">
          <Link to="/ai-agent" className="bg-white rounded-xl shadow-lg p-6 flex flex-col items-center hover:bg-green-50 transition block h-full">
      ...
      {/* Step-by-step Guide */}
      <motion.div 
        className="max-w-3xl mx-auto bg-white/80 rounded-2xl shadow-xl p-8 mb-12"
