...
app.post('/auth/register', async (req, res) => {
    try {
        ...
        // Generate JWT token
        const token = jsonwebtoken_1.default.sign({ userId: userRef.id, email, role: 'farmer' }, JWT_SECRET, { expiresIn: '7d' });
        res.status(201).json({
...
app.post('/auth/login', async (req, res) => {
    try {
        ...
        // Generate JWT token
        const token = jsonwebtoken_1.default.sign({ userId: userDoc.id, email: userData.email, role: userData.role }, JWT_SECRET, { expiresIn: '7d' });
        // Update last login
...
app.get('/weather/:city', async (req, res) => {
    ...
    try {
        ...
        const response = await axios_1.default.get(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${API_KEY}&units=metric`);
        const weatherData = {
        ...
        res.status(500).json({
            ...
            message: ((_b = (_a = error === null || error === void 0 ? void 0 : error.response) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.message) || 'Weather service unavailable'
        });
    }
});
// Crops Routes
...
app.post('/crops', authenticateToken, async (req, res) => {
    try {
        const cropData = Object.assign(Object.assign({}, req.body), { userId: req.user.userId, createdAt: admin.firestore.FieldValue.serverTimestamp(), updatedAt: admin.firestore.FieldValue.serverTimestamp() });
        const cropRef = await db.collection('crops').add(cropData);
        res.status(201).json({
            success: true,
            message: 'Crop added successfully',
            data: Object.assign({ id: cropRef.id }, cropData)
        });
    }
    catch (error) {
...
app.get('/schemes', async (req, res) => {
    try {
        ...
        const schemes = schemesSnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
...
app.get('/market/prices', async (req, res) => {
    try {
        ...
        const prices = pricesSnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
...
app.get('/users/profile', authenticateToken, async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            data: Object.assign({ id: userDoc.id }, userData)
        });
    }
    catch (error) {
...
app.post('/community/posts', authenticateToken, async (req, res) => {
    try {
        const postData = Object.assign(Object.assign({}, req.body), { userId: req.user.userId, userEmail: req.user.email, likes: 0, comments: [], createdAt: admin.firestore.FieldValue.serverTimestamp(), updatedAt: admin.firestore.FieldValue.serverTimestamp() });
        const postRef = await db.collection('communityPosts').add(postData);
        res.status(201).json({
            success: true,
            message: 'Post created successfully',
            data: Object.assign({ id: postRef.id }, postData)
        });
    }
    catch (error) {
